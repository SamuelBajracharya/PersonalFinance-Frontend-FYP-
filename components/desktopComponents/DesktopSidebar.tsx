...
const DesktopSidebar = () => {
  ...
  const mainNav = [
    ...
    { href: '/analytics', label: 'Analytics', icon: <AiFillPieChart size={20} /> },
    {
      href: '/transactions',
      label: 'Transactions',
      icon: <img src="/transactionHistory.svg" alt="Transaction History" className="w-5 h-5" />,
    },
    { href: '/budgetgoals', label: 'Budget Goals', icon: <FaPiggyBank size={20} /> },
    {href: '/aiadvisor', label: 'AI Advisor', icon: <PiChatCircleTextFill size={22} />},
    { href: '/rewards', label: 'Rewards', icon: <IoTrophy size={18} /> },
  ]

  const supportNav = [
    { href: '/help', label: 'Help & Support', icon: <IoMdHelpCircle size={22} /> },
  ]
        ...
        {mainNav.map((item) => {
          ...
          return (
            <button
              ...
              className={`flex items-center gap-3 w-full p-2 rounded-full transition-colors
                ${isActive
