...
export default function BudgetGoals() {
    ...
    const { data: explanation } = useBudgetPredictionExplanation(effectiveBudgetId);
    ...
    const predictedOverrun = Math.max(projectedSpend - budgetAmount, 0);
    const headline = selectedStatus?.predicted_to_exceed
        ? `Predicted overspend: ${currency(predictedOverrun)}`
        : `On track with ${currency(selectedStatus?.remaining_budget)} remaining`;
